...
async def test_submit_query_in_session(client: AsyncClient) -> None:
    ...
    resp = await client.post(
        f"/api/v1/conversations/{session_id}/query",
...
async def test_session_not_found(client: AsyncClient) -> None:
    resp = await client.post(
        "/api/v1/conversations/nonexistent/query",
        json={"question": "test"},
    )
...
async def test_session_history(client: AsyncClient) -> None:
    ...
    # Get history
    resp = await client.get(f"/api/v1/conversations/{session_id}/history")
    assert resp.status_code == 200
    data = resp.json()
...
async def test_session_isolation(client: AsyncClient) -> None:
    ...
    # Create two sessions
    r1 = await client.post("/api/v1/conversations")
    r2 = await client.post("/api/v1/conversations")
    s1 = r1.json()["session_id"]
    s2 = r2.json()["session_id"]

    # Submit query only in session 1
    await client.post(
        f"/api/v1/conversations/{s1}/query",
        json={"question": "How many users are there?"},
    )

    # Session 2 should have no queries
    resp = await client.get(f"/api/v1/conversations/{s2}/history")
